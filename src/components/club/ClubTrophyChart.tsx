...
export function ClubTrophyChart({ members, playerTag, progress, isLoading }: ClubTrophyChartProps) {
  ...
  const lines = useMemo(() => {
    return members
      .filter(m => m.loaded && m.progression.length > 0)
      ...
      .map((m, i) => ({
        ...
        progression: [0, ...m.progression], // start at zero
      }))
  }, [members, playerNorm])
  ...
  // Auto-init: show top 8 + player
  const activeSet = useMemo(() => {
    if (visible !== null) return visible
    if (lines.length < 3) return new Set(lines.map(l => l.tag))
    const top = new Set(lines.slice(0, 8).map(l => l.tag))
    const pl = lines.find(l => l.tag.toUpperCase() === playerNorm)
    if (pl) top.add(pl.tag)
    return top
  }, [visible, lines, playerNorm])

  const visibleLines = lines.filter(l => activeSet.has(l.tag))

  // Find the max number of battles across visible lines
  const maxLen = useMemo(() => Math.max(...visibleLines.map(l => l.progression.length), 2), [visibleLines])

  // Geometry
  const geo = useMemo(() => {
    const allVals = visibleLines.length > 0 ? visibleLines.flatMap(l => l.progression) : [0, 10, -10]
  ...
  const { toX, toY, zeroY, yLabels } = geo
    ...
    <div className="brawl-card-dark border-[#090E17] overflow-hidden">
      ...
      <div className="w-full overflow-hidden rounded-xl bg-[#0A101D] border border-white/5">
        <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full h-auto">
          ...
          {visibleLines.map(line => {
            const pts = line.progression.map((v, i) => ({ x: toX(i, line.progression.length), y: toY(v) }))
            ...
            return (
              <g key={line.tag}>
                {/* Area fill */}
                <path d={`${d} L ${last.x} ${zeroY} L ${pts[0].x} ${zeroY} Z`} fill={`url(#${uid}-${line.tag.replace('#', '')})`} />
                {/* Glow line */}
                <path d={d} fill="none" stroke={line.color} strokeWidth={isPlayer ? 6 : 4} strokeOpacity={0.25} strokeLinecap="round" strokeLinejoin="round" filter={isPlayer ? `url(#${uid}-glow)` : undefined} />
                {/* Main line */}
                <path d={d} fill="none" stroke={line.color} strokeWidth={isPlayer ? 2.5 : 1.5} strokeLinecap="round" strokeLinejoin="round" />
                {/* Intermediate dots */}
                {pts.slice(0, -1).map((p, di) => (
                  <circle key={di} cx={p.x} cy={p.y} r={isPlayer ? 3 : 2} fill={line.color} stroke="#0A101D" strokeWidth={1.5} opacity={0.6} />
                ))}
                {/* End dot (larger, with glow) */}
                <circle cx={last.x} cy={last.y} r={isPlayer ? 6 : 4} fill={line.color} stroke="#0A101D" strokeWidth={2} />
                {isPlayer && <circle cx={last.x} cy={last.y} r={10} fill="none" stroke={line.color} strokeWidth={1.5} strokeOpacity={0.3} />}
                {/* End value label */}
                <text x={last.x + (isPlayer ? 14 : 8)} y={last.y + 4} fill={line.color} fontSize={isPlayer ? 11 : 9} fontFamily="'Lilita One', sans-serif" fontWeight={isPlayer ? 700 : 400}>
      ...
      <div className="mt-4">
        <div className="flex gap-2 mb-3">
          <button onClick={() => setVisible(new Set(lines.map(l => l.tag)))} className="text-[10px] font-bold text-[#4EC0FA] bg-[#4EC0FA]/10 px-3 py-1 rounded-full hover:bg-[#4EC0FA]/20 transition-colors">All</button>
          ...
          <button onClick={() => { const t = new Set(lines.slice(0, 5).map(l => l.tag)); setVisible(t) }} className="text-[10px] font-bold text-green-400 bg-green-500/10 px-3 py-1 rounded-full hover:bg-green-500/20 transition-colors">Top 5</button>
          <button onClick={() => { const t = new Set(lines.slice(-5).map(l => l.tag)); setVisible(t) }} className="text-[10px] font-bold text-red-400 bg-red-500/10 px-3 py-1 rounded-full hover:bg-red-500/20 transition-colors">Bottom 5</button>
        </div>
