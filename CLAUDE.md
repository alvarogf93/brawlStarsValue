...
Auditoría completa en `docs/audits/2026-04-30-adsense/README.md`; rollout en branch `adsense/full-rollout`.
**Estado actual:**
- ✅ Fase 1 (compliance crítica): cosmetics + share sin slots, stats reducido a 1 slot, `/methodology` + `/about` publicados con ≥1500 / ≥400 palabras × 13 locales, footer site-wide linka ambas, tests anti-regresión en `src/__tests__/integration/adsense-forbidden-pages.test.ts`, `battles` gate ≥10 partidas (AD-09).
- ⚠️ Fase 2 (monetización compliant): `/brawler/[id]`, `/brawler`, `/picks` se reescribieron como artículos con 2-3 slots compliant cada uno.
- ⚠️ Fase 3 (sostenibilidad): `/battle-history`, `/leaderboard` con contexto editorial; test E2E ad-content-ratio.
**Do NOT solicit AdSense review hasta que las 3 fases estén en `main` y deployed ≥14 días.** Una rejection prematura cuenta contra el próximo intento.
...
- **Ads go through `SafeAdSlot`, not `AdPlaceholder` directly** — `SafeAdSlot` (`src/components/ui/SafeAdSlot.tsx`) forces every callsite to pass a required `hasContent: boolean` prop; it returns `null` when false. The post-2026-04-30 sanción audit (`docs/audits/2026-04-30-adsense/`) hardened the rules: **públicas con ads** = `/brawler/[id]` (1 slot, Fase 2.1 ampliará a 2-3), `/brawler` roster (1 slot post Fase 2.2), `/picks`, `/battle-history`, `/leaderboard`. **Privadas free-user** = `/profile/[tag]` (2 slots), `/profile/[tag]/battles` (gate `>= 10` batallas — AD-09), `/profile/[tag]/brawlers`, `/profile/[tag]/club` (2 slots), `/profile/[tag]/stats` (1 slot — reducido de 3). **Explícitamente SIN ads (forbidden)**: `/profile/[tag]/cosmetics` (calculadora), `/profile/[tag]/share` (acción puntual), landing, subscribe, login, error pages, loading/skeleton states, `/methodology`, `/about`. Reglas duras: (1) cada página con ads ≥ 300 palabras visibles above-the-fold; (2) calculadoras / formularios / share-cards / acciones puntuales nunca llevan ads; (3) máximo 1 slot por viewport visible (stats reducido de 3→1); (4) cada página con ads tiene `<h1>` propio + estructura semántica; (5) datos puramente agregados de terceros (Supercell/Brawlify) no bastan — añadir comentario, metodología, ejemplos propios. Hay un test (`src/__tests__/integration/adsense-forbidden-pages.test.ts`) que falla si cosmetics/share vuelven a importar `SafeAdSlot` o si stats acumula >1 slot. Trial y premium se gatean vía `isPremium(profile)`. Nunca pasar literal `true` a `hasContent` — derrota el diseño defensivo.
...
- **Brawler 7d WR trend is precomputed, not computed per request** — `supabase/migrations/020_*` + `021_*` + `022_*` create `public.brawler_trends` (one row per brawler) and a `compute_brawler_trends()` function scheduled on pg_cron `17 */6 * * *`. The endpoint `/api/meta/brawler-trends` reads this table with a freshness filter (`computed_at > now-12h`), falling through to an inline paginated scan when the fast path is unavailable. Two reasons we do it this way: (1) scanning 10 k rows on every ISR refresh doesn't scale past ~200 k-row `meta_stats`; (2) PostgREST's 1000-row cap (see gotchas) silently corrupted the inline path until paginated. If you add new precomputed analytics, copy this pattern — tiny table + pg_cron + heartbeat + freshness guard + inline fallback — rather than scanning on the hot path.
- **`meta_stats` aggregation MUST filter `source='global'`** — `meta_stats` is multi-source: `'global'` (cron meta-poll, top pro players) and `'users'` (cron sync, premium users' personal data). Any "PRO trend / WR / popularity" served to anonymous visitors must filter by source or it leaks personal data into the public meta. Migration `022_*` patched `compute_brawler_trends()`; `/api/meta/pro-analysis` already had the filter. If you add a new aggregator over `meta_stats`, apply the filter — the historical bug (LOG-01/LOG-04) was that the SQL function and the TS inline fallback both omitted it.
- **`compute7dTrend` logic lives in TWO places and MUST stay in sync** — `src/lib/brawler-detail/trend.ts` (TypeScript, per-brawler detail endpoint) and `public.compute_brawler_trends()` in `supabase/migrations/022_*` (SQL, bulk precomputation; supersedes the function body in 020/021). Both use `MIN_BATTLES_PER_TREND_WINDOW = 3`, a 7d/14d split, and `source='global'`. If you change the threshold, the window math, the rounding rule, or the source filter in one, change it in the other or the bulk endpoint and the detail endpoint will silently disagree for the same brawler. There's an integration test covering the endpoint's fast/fallback parity and one asserting the source filter on the inline path, but nothing enforces SQL-vs-TS equivalence — that's a manual discipline until we ship a Postgres-backed test environment.
